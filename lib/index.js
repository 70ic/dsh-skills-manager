...
export function parseState(text) {
    ...
    const state = { version: 1, projects: record.projects };
    if (record.policy !== undefined) {
        ...
        const { mode, names } = record.policy;
 ...
 */
export function resolvePolicy(runtime, settingsValue, entry) {
...
export function parsePolicyNames(tokens) {
    const names = [];
...
class SkillManager {
    ...
    control;
    disposed = false;
    constructor(ctx, entry, stateFile) {
        this.ctx = ctx;
        this.entry = entry;
    ...
    provider = {
        ...
        list: (options) => {
            ...
            if (mode === 'all' && overrides === undefined)
    ...
    scheduleRefresh(cwd) {
        ...
        void this.refresh(cwd)
            .catch((error) => { this.ctx.logger.warn(`skills-manager: universe refresh failed: ${String(error)}`); })
    ...
    async setPolicy(mode, names) {
        for (const skillName of names) {
            if (!isSkillName(skillName))
                throw new Error(`"${skillName}" is not a kebab-case skill name`);
        }
        const state = this.loadState();
    ...
    async clearPolicy() {
        ...
        if (state.policy === undefined)
            return;
        const next = { version: 1, projects: state.projects };
    ...
    ctx.inject(['commands'], (commandCtx) => {
        commandCtx.commands.register({
            ...
            handler: async ({ agent, rawInput }) => {
                ...
                if (action === 'mode')
                    return await handleModeCommand(manager, tokens.slice(1));
                if ((action === 'enable' || action === 'disable' || action === 'reset') && arg !== undefined && tokens.length === 2) {
                    if (!isSkillName(arg))
                        return { kind: 'error', text: `"${arg}" is not a kebab-case skill name` };
                    try {
...
async function handleModeCommand(manager, tokens) {
    ...
    try {
        if (subcommand === 'reset') {
            if (tokens.length !== 1)
                return { kind: 'error', text: 'Usage: /skills mode reset' };
